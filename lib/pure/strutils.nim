...
macro allEnumValues(typ: typedesc): untyped =
  # Generates a set of all enum values.
  # It checks if the given enum contains multiple fields with the same string value.
  ...
  for f in impl:
    ...
    of nnkEnumFieldDef:
      ...
      of nnkTupleConstr: fStr = f[1][1].strVal
      of nnkIntLit: fStr = f[0].strVal
...
proc parseEnum*[T: enum](s: string): T =
  ...
  let allEnums = allEnumValues(T)
  for e in allEnums:
    if cmp(nimIdentNormalize(s), nimIdentNormalize($e)) == 0:
      return e
